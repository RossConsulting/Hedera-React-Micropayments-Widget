...
import "arrive";
// import hBarIcon from "./hbar-icon-white.png"; Need to check how to import assets like these in nwb
const hBarIcon = ""; // TODO: Placeholder

export default class HederaMicropayment extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      hasPlugin: false,
      ...
      if (id != null && status == null) {
        ...
        if (status === "SUCCESS") {
          localStorage.setItem(
            "hedera" + id,
            JSON.stringify({ paymentSuccessful: true, message: "SUCCESS" })
          );
          window.close();
          ...
          localStorage.setItem(
            "hedera" + id,
            JSON.stringify({ paymentSuccessful: false, message: "NO_ACCOUNT" })
          );
  ...
  payWithHedera(
    ...
    const paramId =
      ([...Array(7)].map(_ => ((Math.random() * 36) | 0).toString(36)).join`` +
        new Date() / 1e3) |
      0;
    const url =
    ...
    features.push(
      `top=${window.top.outerWidth / 2 + window.top.screenY - 640 / 2}`
    );
  ...
  render() {
    if (this.state.isOnPopupPage) {
      return (
        <div className="hedera-process-payment-popup">
          {!this.state.invalidRequest &&
            this.state.hasPlugin &&
            !this.state.handlingRedirect && (
              <React.Fragment>
                {" "}
                <HederaExtensionTag
                ...
                />
                <div className="hedera-waiting-image-wrapper-no-anim">
                  {" "}
                  <img src={hBarIcon} alt="Hello future" />
                </div>
                <h4
                  style={{
                    paddingTop: "1rem"
                ...
                >
                  Sending payment.Do not close this page.Please wait ...
                </h4>{" "}
              </React.Fragment>
            )}
          {!this.state.invalidRequest &&
            ...
            this.state.handlingRedirect && (
              <React.Fragment>
                ...
                >
                  Processing payment.Do not close this page.Please wait ...
                </h4>{" "}
              </React.Fragment>
            )}
          {this.state.invalidRequest && this.state.hasPlugin && (
            <div>
              Payment data not found. Please try again. This page will close in
              3 seconds.
            </div>
          )}
          {!this.state.hasPlugin && (
            <div>Chrome Hedera Browser Extension is required.</div>
          )}
        </div>
      );
    }
    ...
    return (
      <React.Fragment>
        ...
        {this.state.processingPayment && (
          <div className="hedera-waiting-modal">
            <div className="hedera-waiting-image-wrapper">
              <img src={hBarIcon} alt="Hello future" />
            </div>
            <h4
              style={{
        ...
        <HederaExtensionTag
          recipient="0.0.1742"
          amount={this.props.maximumAmount}
          type="maximum"
          memo="MAXIMUM"
          pathname="/"
        />{" "}
...
export async function detectPlugin() {
  return await fetch(
    "chrome-extension://ligpaondaabclfigagcifobaelemiena/icons/icon16.png"
  );
}
