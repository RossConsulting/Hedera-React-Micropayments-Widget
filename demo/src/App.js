  ...
  handlePayment(amount, memo, paymentIndex) {
    this.setState({
      payments: [
        ...
        ...this.state.payments.slice(
          paymentIndex + 1,
          this.state.payments.length
        )
      ]
    });
    this.props.payWithHedera(
      amount,
      "0.0.1742",
      "valid payment",
      "aRandomIDForContent",
      ...
      <div className="App">
        ...
        <ul>
          {this.state.payments.map((p, i) => (
            <li key={i}>
              <div>
                {`Payment amount: ${p.amount} tinybars | Status: ${
                  p.paid ? "PAID" : p.pending ? "PROCESSING" : "UNPAID"
                }${p.note !== "" ? " | " + p.note : ""}`}
              </div>
              {!p.pending && !p.paid && (
                <button
                  onClick={this.handlePayment.bind(this, p.amount, p.memo, i)}
                >
                  Pay Now
                </button>
              )}
